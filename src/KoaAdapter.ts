...
type KoaRouteMethods = KoaRouter['all' | 'delete' | 'get' | 'head' | 'options' | 'patch' | 'post' | 'put']
type KoaHandler = RequestHandler<Koa.Request, Koa.Response>

export class KoaAdapter extends AbstractHttpAdapter<http.Server | https.Server, Koa.Request, Koa.Response> {
	...
	public delete(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		...
		return this.router.delete(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public get(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.get(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public head(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.head(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public options(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.options(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public patch(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.patch(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public post(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.post(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	public put(pathOrHandler: string | KoaHandler, handler?: KoaHandler): any {
		const [routePath, routeHandler] = this.getRouteAndHandler(pathOrHandler, handler);

		return this.router.put(routePath, (ctx: Koa.Context, next: Koa.Next) => routeHandler!(ctx.request, ctx.response, next));
	}

	private getRouteAndHandler(pathOrHandler: string | KoaHandler, handler?: KoaHandler): [string, KoaHandler] {
	...
	public close() {
		return new Promise(
			(resolve) => this.httpServer.close(resolve)
		);
	}
	...
	public setErrorHandler(handler: (err: Error, ctx: Koa.Context) => void, prefix?: string): any {
	...
	public createMiddlewareFactory(requestMethod: RequestMethod): (path: string, callback: Function) => any {
		return (path: string, callback: Function) => {
			...
			const routeMethod = (routeMethodsMap[requestMethod] || routeMethodsMap[RequestMethod.GET]).bind(this.router);

			return routeMethod(path, (ctx: Koa.Context, next: Koa.Next) => callback(ctx.request, ctx.response, next));
		};
	}
}
